...
const checkSession = function checkSession(req, res, next) {
  ...
  return Promise.try(async () => {
    ...
    return client.get(
      `${request_payload.email}:${user_type}`,
      async (err, result) => {
        try {
          if (result) {
            ...
            if (
              actual_payload.iat !== request_payload.iat ||
...
export const companyCheckSession = function companyCheckSession(
  req,
  res,
  next,
) {
  ...
  return Promise.try(async () => {
    ...
    if (!isFound)
      return res.status(401).json({
        status: 401,
        error:
          'Not Authorized. Please contact support to lay complains. contact support@koogah.com',
      })
    if (!isFound.is_approved)
      return res.status(401).json({
        status: 401,
        error:
          'Cannot perform this action until your account is approved by our team',
      })
  ...
  return Promise.try(async () => {
    ...
    if (!isFound.is_verified)
      return res.status(401).json({
        status: 401,
        error:
          'Cannot perform this action until you have verified your account. Please check your email for a verification link sent to you',
      })
    if (payload.is_courier) {
      return res.status(401).json({
        status: 401,
...
export const externalApiSession = (req, res, next) => {
  return Promise.try(async () => {
    ...
    if (
      __koogah_external_api_session_secret !==
      process.env.EXTERNAL_API_SESSION_SECRET
    ) {
      return res.status(400).json({
        status: 400,
        error: 'Not Authorized to use this API',
