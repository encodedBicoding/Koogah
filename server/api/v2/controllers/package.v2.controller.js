...
import {sendNewPackageNotification} from '../../v1/helpers/slack'
...
class PackageV2 {
  ...
  static async request_dispatch_v2(req, res) {
    return Promise.try(async () => {
      ...
      let {type} = req.params
      let {transport_mode_category, delivery_price, distance, value, ...data} =
        req.body
      ...
      const message = {
        ...
        detail: `new ${data.is_express_delivery ? 'EXPRESS' : ''} pickup @ ${
          data.pickup_address
        } to ${data.dropoff_address} for ₦${new Intl.NumberFormat(
          'en-US',
        ).format(
          Number(delivery_price),
      ...
      let comp_m = `New ${data.is_express_delivery ? 'EXPRESS' : ''} pickup @ ${
        data.pickup_address
      ...
      return res.status(200).json({
        status: 200,
        message:
          'Package created successfully. Please wait, dispatchers will reach out to you soon',
        data: package_detail,
      })
    }).catch((err) => {
      log(err)
  ...
  static change_price(req, res) {
    return Promise.try(async () => {
      ...
      const NEW_NOTIFICATION = {
        ...
        action_link: isProduction
          ? `${process.env.SERVER_APP_URL}/package/owner/view/${package_id}`
          : `http://localhost:4000/v1/package/owner/view/${package_id}`, // ensure customer is logged in
      }
      ...
      return res.status(200).json({
        status: 200,
        message:
          'package delivery price change awaits approval from package owner',
    ...
    return Promise.try(async () => {
      ...
      if (_package.customer_id !== user.id) {
        return res.status(400).json({
          status: 401,
          error:
            'You are not allowed to perform this action. Please contact support@koogah.com',
        })
      }
      if (!_package.pending_delivery_price) {
        return res.status(400).json({
          status: 400,
          error:
            'Oops, seems there is no pending weight change for this package',
        })
      }
      ...
      if (response === 'approve') {
        if (_package.payment_mode === 'virtual_balance') {
          ...
          if (
            Number(_package.pending_delivery_price) >
            Number(user.virtual_balance)
          ) {
            return res.status(400).json({
              status: 400,
              error:
                'Please top-up your account to approve this new weight change',
            })
          }
          ...
          if (
            Number(user.virtual_balance) - Number(updated_V_A_B) <
            Number(_package.pending_delivery_price)
          ) {
            return res.status(400).json({
              status: 400,
              error:
                'Please top-up your account to approve this new price change',
            })
          }
          updated_V_A_B =
            Number(updated_V_A_B) + Number(_package.pending_delivery_price)
          await Customers.update(
            {
              virtual_allocated_balance: updated_V_A_B,
            },
        ...
        NEW_NOTIFICATION.action_link = isProduction
          ? `${process.env.SERVER_APP_URL}/package/preview/${package_id}`
          : `http://localhost:4000/v1/package/preview/${package_id}` // ensure courier is logged in
      }
      if (response === 'decline') {
        await Packages.update(
          {
      ...
      const res_message = `Successfully ${
        response === 'approve' ? 'approved' : 'declined'
                ...
                if (!_package.multiple_delivery_id) {
                  ...
                  if (
                    ...
                    return res.status(400).json({
                      status: 400,
                      error:
                        'Please top-up your account to successfully edit this package',
                    })
                  }
    ...
    return Promise.try(async () => {
      ...
      const packagesRes = rows.map((row) => {
        ...
        return {
          ...
          ...rowData,
          delivery_price: user.is_cooperate
            ? getCorporatePriceSlashed(rowData.delivery_price)
            : getIndividualPriceSlashed(rowData.delivery_price),
        }
      })
